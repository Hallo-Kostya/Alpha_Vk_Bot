...
async def ack_choosen_date(message: Message):
    ...
    if response == "да":
        ...
        await message.answer(
            "Успешно создали интервью! Ссылка на встречу придет Вам в этот же чат, спасибо за уделённое время. Если что, Вы всегда можете активировать меня, написав 'Начать'."
        )
        return
    else:
        await state_dispenser.set(
